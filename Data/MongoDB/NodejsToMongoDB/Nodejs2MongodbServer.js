...
// 1、參考資料：
// Airbnb JavaScript Style「https://github.com/airbnb/javascript」(代碼書寫遵循規則);
// https://nodejs.org/en/ 官方説明文檔;
// http://nodejs.cn/ 中文説明文檔;
// https://www.npmjs.com/ 包管理工具「npm」官方説明;
// https://registry.npm.taobao.org/ 淘寶的「npm」國内鏡像，使用命令：「npm install --registry=https://registry.npm.taobao.org/」,使用命令：「npm config set registry https://registry.npm.taobao.org/」永久更改npm下載源;
// http://expressjs.com/ 第三方快速 Web 框架「express」搭建服務器;
// https://github.com/expressjs/body-parser 第三方擴展包「express」的中間件「body-parser」用於獲取客戶端瀏覽器post請求發送的數據;
// https://github.com/expressjs/session 第三方擴展包「express」的中間件「express-session」用於保存用戶端登陸狀態;
// https://www.npmjs.com/package/cookie-parser 第三方擴展包「express」的中間件「cookie-parser」用於保存用戶端登陸狀態;
// https://aui.github.io/art-template/zh-cn/index.html 第三方擴展包「art-template」模板引擎用於渲染網頁代碼;
// https://github.com/aui/express-art-template 第三方擴展包「express-art-template」用於在express框架中使用art-template模板渲染網頁;
...
// https://www.bootcss.com/ 前端開發框架;
// https://developer.mozilla.org/zh-CN/ 網站提供開放網絡（Open Web）技術有關的信息，幫助文檔;
// https://github.com/segmentio/nightmare 「nightmare」網頁自動化測試框架;
// https://getbootstrap.com/docs/4.3/content/reboot/ bootstrap4.3 官方文檔
// https://api.jquery.com/ jquery 官方文檔
// https://github.com/visionmedia/superagent SuperAgent官方倉庫
// https://www.npmjs.com/package/blueimp-md5 賬號傳輸加密算法包md5官方文檔;
// https://github.com/SheetJS/js-xlsx 瀏覽器JavaScript操作Excel插件倉庫
// http://demo.haoji.me/2017/02/08-js-xlsx/ 瀏覽器JavaScript操作Excel示例
// https://oss.sheetjs.com/js-xlsx/ 瀏覽器JavaScript操作Excel示例
// https://github.com/ServiceStack/redis-windows 内存數據庫 Redis windows版倉庫
// https://redis.io/download 内存數據庫 Redis 官網
// http://redis.cn/ 内存數據庫 Redis 中文網站
// https://www.npmjs.com/package/rio 與 R 語言 Rserve 服務器通訊擴展包
// https://github.com/albertosantini/node-rio 與 R 語言 Rserve 服務器通訊擴展包
...
// 2.2 載入第三方擴展包（需要先在Windows控制臺（CMD），使用「npm install」方法安裝成功，才能引入使用）
...
// 2.3 載入自定義的其它模塊（模塊前需要寫明載入路徑）
// const Router = require('./router.js') // 當加載自定義的模塊時，引用模塊需要包括路徑和模塊名的完整引用，只寫模塊名會報錯;
// const PortIsOccupied_exports = require('./PortIsOccupied.js'); // 加載自己封裝的模塊「PortIsOccupied.js」;
// const PortIsOccupied_exports_CheckString = PortIsOccupied_exports.CheckString(); // 使用「PortIsOccupied.js」模塊中的成員「CheckString(letters, fork)」函數, 用於檢查字符串中的字符類型（使用正則函數的方法）以檢驗用戶輸入參數的合規性;

// console.log(`${OS.platform()},${OS.hostname()},${IP.address()}`); //查看服務器系統信息用於調試;


// 二、創建 web server 服務器接收到客戶端請求時的具體響應操作;
...
// 二、使用 Node.js 原生的 http 庫創建 web server 服務器;
...
// 二、使用 Node.js 原生的 http 庫創建 web client 客戶端;
